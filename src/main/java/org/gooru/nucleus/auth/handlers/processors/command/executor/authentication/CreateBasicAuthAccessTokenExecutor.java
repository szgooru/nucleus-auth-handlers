...
public final class CreateBasicAuthAccessTokenExecutor implements DBExecutor {
  ...
  public void validateRequest() {
    ...
    rejectIfNullOrEmpty(authClientDTO.getClientId(), MessageCodeConstants.AU0001, HttpConstants.HttpStatus.UNAUTHORIZED.getCode());
    rejectIfNullOrEmpty(authClientDTO.getClientKey(), MessageCodeConstants.AU0002, HttpConstants.HttpStatus.UNAUTHORIZED.getCode());
    ...
    reject((authClient.getGrantTypes() == null || !authClient.getGrantTypes().contains(authClientDTO.getGrantType())), MessageCodeConstants.AU0005,
    ...
    if (username.indexOf("@") > 1) {
      LazyList<AJEntityUserIdentity> userIdentityEmail  = AJEntityUserIdentity.where(AJEntityUserIdentity.GET_BY_EMAIL_PASSWORD, username, password);
      userIdentity = userIdentityEmail.size() > 0 ? userIdentityEmail.get(0) : null;
    } else {
      LazyList<AJEntityUserIdentity> userIdentityUsername  = AJEntityUserIdentity.where(AJEntityUserIdentity.GET_BY_USERNAME_PASSWORD, username, password);
  ...
  public MessageResponse executeRequest() {
    ...
    LazyList<AJEntityUserPreference> userPreferences = AJEntityUserPreference.where(AJEntityUserPreference.GET_USER_PREFERENCE, userIdentity.getUserId());
