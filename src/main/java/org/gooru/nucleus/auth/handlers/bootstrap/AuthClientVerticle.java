...
public class AuthClientVerticle extends AbstractVerticle {
    private static final Logger LOG = LoggerFactory.getLogger(AuthClientVerticle.class);
    ...
    public void start(Future<Void> voidFuture) throws Exception {
        ...
        eb.consumer(MessagebusEndpoints.MBEP_AUTH_CLIENT, message -> {
            LOG.debug("Received message: " + message.body());
            vertx.executeBlocking(future -> {
                MessageResponse result =
                    new ProcessorBuilder(ProcessorHandlerType.AUTH_CLIENT, message).build().process();
            ...
            if (result.succeeded()) {
                ...
                LOG.error("Error registering the auth client handler. Halting the auth client machinery");
