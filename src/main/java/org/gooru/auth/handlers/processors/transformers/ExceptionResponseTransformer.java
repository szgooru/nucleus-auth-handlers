...
public class ExceptionResponseTransformer implements ResponseTransformer {
  ...
  public ExceptionResponseTransformer(Error error, int httpStatusCode, boolean isValidationError) {
    this.error = error;
  ...
  private void processTransformation() {
    if (!this.transformed) {
      ...
      if (isValidationError) {
        transformedOutput.put(MessageConstants.MSG_OP_STATUS, MessageConstants.MSG_OP_STATUS_VALIDATION_ERROR);
      } else {
        transformedOutput.put(MessageConstants.MSG_OP_STATUS, MessageConstants.MSG_OP_STATUS_ERROR);
      }
  ...
  private JsonObject getTransformedResponse() {
    ...
    transformedResponse.put(MessageConstants.MSG_HTTP_STATUS, httpStatusCode);
  ...
  private JsonObject getHttpBody() {
    if (isValidationError) {
      return new JsonObject().put(MessageConstants.MSG_HTTP_VALIDATION_ERROR, error);
    } else {
      return new JsonObject().put(MessageConstants.MSG_HTTP_ERROR, error);
    }
  }
}
