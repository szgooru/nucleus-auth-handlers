...
import org.gooru.auth.handlers.processors.command.executor.StateCommandExecutor;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class StateVerticle extends AbstractVerticle {
  static final Logger LOG = LoggerFactory.getLogger(StateVerticle.class);

  @Override
  public void start(Future<Void> voidFuture) throws Exception {
    EventBus eb = vertx.eventBus();

    eb.consumer(MessagebusEndpoints.MBEP_STATE, message -> {
      LOG.debug("Received message: " + message.body());
        vertx.executeBlocking(future -> {
          JsonObject result = new ProcessorBuilder(StateCommandExecutor.class, message).build().process();
      ...
      if (result.succeeded()) {
        LOG.info("State end point ready to listen");
      } else {
        LOG.error("Error registering the state handler. Halting the state machinery");
